...
const elements = {
  ...
  unrecognizedKeywordsList: document.getElementById("unrecognizedKeywordsList"),
...
function bindEvents() {
  ...
  elements.keywordGroups.addEventListener("click", (event) => {
    ...
    if (removeBtn) {
      removeKeyword(removeBtn.dataset.removeGroup, removeBtn.dataset.keyword);
      return;
    }
    ...
    if (addBtn) {
      ...
      const input = elements.keywordGroups.querySelector(`input[data-input-group="${escapeCss(group)}"]`);
...
function saveSettings() {
  ...
  state.settings = next;
  sessionStorage.setItem(STORAGE_KEYS.settings, JSON.stringify(state.settings));
}
...
function removeKeyword(group, keyword) {
  const list = state.settings.keywordGroups[group] || [];
  state.settings.keywordGroups[group] = list.filter((item) => item.toLowerCase() !== keyword.toLowerCase());
  persistSettings();
  renderKeywordGroups();
}
...
async function handleCalendarUpdate() {
  ...
  try {
    ...
    const classified = classifyEvents(parsedEvents, state.settings.keywordGroups);
...
function renderMonthlyCalendars(events, range) {
  const migraineEvents = events.filter((event) => event.groups.includes("Migraine"));
  ...
  if (!months.length) {
    elements.monthlyCalendars.innerHTML = '<span class="helper">No months in selected range.</span>';
    return;
  }

  elements.monthlyCalendars.innerHTML = months
    .map(({ year, month }) => renderMonthCard(year, month, dayCounts, maxCount))
    .join("");
}
...
function renderMonthCard(year, month, dayCounts, maxCount) {
  ...
  const title = firstDay.toLocaleDateString(undefined, { month: "long", year: "numeric" });
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    dayCells.push(`<div class="${className}" style="background:${color}" title="${iso}: ${count} migraine event(s)">${day}${badge}</div>`);
  }
...
function renderUnrecognizedKeywords(events) {
  ...
  events.forEach((event) => {
    const terms = Array.isArray(event.unmatchedTerms) ? event.unmatchedTerms : extractEventTerms(event.title, "");
    terms.forEach((term) => {
  ...
  if (!items.length) {
    elements.unrecognizedKeywordsList.innerHTML = '<span class="helper">No unrecognized terms in this range.</span>';
    return;
  }

  elements.unrecognizedKeywordsList.innerHTML = items
    .map(([term, count]) => `<span class="keyword-chip plain">${escapeHtml(term)} <span class="count">(${count})</span></span>`)
    .join("");
}

function renderStats(events, range) {
  const migraine = events.filter((event) => event.groups.includes("Migraine")).length;
  const medication = events.filter((event) => event.groups.includes("Medication")).length;
  ...
  elements.statsGrid.innerHTML = cards
    .map((card) => `<article class="stat-card"><div class="label">${card.label}</div><div class="value">${card.value}</div></article>`)
    .join("");
}
...
function renderDailyChart(events, range) {
  ...
  events.forEach((event) => {
    if (event.groups.includes("Migraine")) migraine.set(event.date, (migraine.get(event.date) || 0) + 1);
  });
...
function classifyEvents(events, keywordGroups) {
  ...
  return events.map((event) => {
    ...
    const unmatchedTerms = terms.filter((term) => !allKeywords.some((keyword) => term.includes(keyword)));

    return {
      id: event.id || crypto.randomUUID(),
      date: event.date,
      title: event.title,
      originalText: [event.title, event.description].filter(Boolean).join(" | "),
      groups,
...
function renderAllEventsTable(events) {
  if (!events.length) {
    elements.allEventsTableBody.innerHTML = '<tr><td colspan="3" class="helper">No synced events yet.</td></tr>';
    return;
  }
  ...
  const rows = events
    ...
    .map((event) => {
      ...
      const categories = event.groups && event.groups.length ? event.groups.join(", ") : "Unmatched";
...
function normalizeKeywordGroups(keywordGroups) {
  return Object.entries(keywordGroups).map(([group, keywords]) => ({
    group,
    keywords: keywords.flatMap((word) => splitKeywords(word)).map((word) => word.toLowerCase()),
  }));
}
...
function normalizeCalendarUrl(rawUrl) {
  ...
  if (url.pathname.endsWith(".ics") || url.pathname.includes("/calendar/ical/")) {
    return url.toString();
  }

  const cid = url.searchParams.get("cid");
  if (!cid) {
    throw new Error("Calendar URL must include `cid` or point directly to an .ics feed.");
  }
...
async function fetchCalendarText(sourceUrl) {
  ...
  throw new Error(`Unable to download calendar feed. ${lastError ? lastError.message : ""}`);
}
